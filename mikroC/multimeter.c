...
float cap_value;
unsigned char err_c=0;               //counter flood error
unsigned char timer0;
unsigned char str[15];              //15 byte because FloatToStr returns at least 14 char
...
char* codetxt_to_ramtxt(const char* ctxt){    //function for saving RAM space by copying from ROM to RAM when needed
static char txt[20];
char i;
...
void volt_meter(){
     ...
     while (PORTC.F1==0){
           ...
           get_volt_value(0,1);
           if (adc_value<1017){                              // the maximum voltage we get on ADC pin is less than 5 volt because of voltage drop over cd4066
              //FloatToStr(adc_value,str);                  //for debugging
...
void ok(){
switch (index){
       case 0:delay_ms(150);volt_meter();       //delay to prevent the function from returning immediately
              break;
